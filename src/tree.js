const hash_ = require("./tree-hashgen");
...
const tree = (function (none) {
  ...
  class node {
    // __ = none;
    constructor(root = null, value = null) {
    ...
    id = (id_) => {
      if (none !== id_) {
        ...
        if (has_(idcache_, id_) && this !== idcache_[id_]) {
          // remove id from node that holds it
          cc(idcache_[id_]).id = none;
        }

        // remove old entry from idcache_
        // if provided another id for this node
        if (this === idcache_[oldid_]) {
          delete idcache_[oldid_];
        }
  ...
  class tree extends node {
    ...
    node = (value = null) => { /** TODO -> config {value: null, id: none} */
      return new node(this, value);
    };
    byid = (id) => {
      let node = idcache_[id];
      if (node) return this === node.root() ? node : none;
    };
  }
  ...
  function traverser_(node) {
    this.callback.call(this.context, node);
    node.traverse(this.callback, this.context);
  }
  function query_(node) {
    if (true === this.callback.call(this.context, node)) push_(this.list, node);
  }
